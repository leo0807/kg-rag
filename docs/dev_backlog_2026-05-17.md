...
| P1     | 0      | 0 人天    | 9/9 闭环 ✅（F055/F031/F056/F060/F073/F076/F117/F118/F120）；计数修正（之前 stale=2） |
| P2     | 0      | 0 人天    | F122 已关闭（2026-05-31）；F123 降级 P3（2026-05-31）；**P2 全闭环 ✅** |
| P3     | 8      | 9.5 人天  | F124/F125/F126/F127/F129/F130 原有；F123 降级加入（0.5天）；F128 独立建档加入（0.5天） |
| 合计   | 8      | 9.5 人天  | 较初版 -14条；P0+P1+P2 全闭环达成；P3 剩余 8 条 ~9.5 天 |

> 估时按"单人开发，串行"计，不含 code review 时间。
> 最后更新：2026-05-31（P1 摘要表计数修正：9/9 全部闭环，stale 2→0）。

---

## 🎉 2026-05-31 P0+P1+P2 全闭环里程碑

本轮工作完成 **6 个 P0 + 9 个 P1 + 7 个 P2 共 22 条**主要任务。

**关键交付**：
- F115 LLM 重试 + 熔断（circuit breaker + retry with backoff）
- F116 优雅关闭（SIGTERM / SIGINT 正确处理，任务不 mid-flight cancel）
- F117 PDF 入库 Celery 化（Redis 持久化，worker 端到端验证）
- F122-state 全局任务状态 Redis 迁移（6 服务 `_tasks` dict → RedisTaskStateStore，18 unit tests）
- F122 全局长任务 Celery 化（graph_prediction / backfill / batch_ingest / conflict_scan / objective_doc_eval）
- F056 自动策略路由（query 类型自动分流 parallel/sequential/semantic）
- F079 对话分支（branch create/switch/delete，conversation tree 持久化）
- F107 任务状态修复（sources_count 累加、init_tables 表同步）
- F113 + F114 文件上传校验（MIME + magic bytes + 全局请求大小限制）
- F055 / F060 / F073 / F118 / F120 / F121 等稳定性与名实相符修复

**剩余 P3 长尾 8 条，共约 9.5 人天工作量，下个 sprint 决策处理。**
...
**降级决策（2026-05-31）**：
P2 → P3。纯前端 polish，无依赖，影响面窄（登录页每会话访问一次，退出入口是 UX 便利性而非功能阻塞）。与 F124 知识图谱移动端深度优化同类性质，保持优先级一致。决策于 F122-state 闭环后 backlog 整理时确认。
...
> **本 sprint 跳过原因**：手势库兼容性 + 性能优化路径不确定，
> 估时 timebox 调研可能产出"建议下个 sprint"。
> 本 sprint 专注 7 条确定性任务。F124 保留 P3，留待独立调研。
...
### F128 stream_agent SSE timeout/retry 监控

**状态**：🔴 未实现
**优先级**：P3
**估时**：0.5 天
**审计来源**：F122 实施时识别：stream_agent 在 SSE 实时路径不应 Celery 化，但缺少超时控制和重试机制（2026-05-30）

**任务描述**：
`src/services/agent/stream_agent.py` 在 SSE 流式响应中调用 LLM，属于用户实时路径（不应 Celery 化）。但当前缺少：
1. LLM 调用超时控制（可能挂死）
2. 失败重试（F115 LLM 熔断已覆盖但本路径未对接）
3. 调用指标（平均耗时 / 失败率 / 重试次数）

**改造**：
- 复用 F115 的 `retry.py` 模块（已实现）
- 在 stream_agent.py LLM 调用处用 `call_with_retry` 包装
- 加 timeout 参数（从 `LLM_STREAM_TIMEOUT` 环境变量读取）
- 加结构化日志记录耗时和失败原因

**验收**：
- [ ] SSE 流式 LLM 调用有超时控制
- [ ] 失败时按 F115 重试策略（retriable codes only）
- [ ] 日志记录耗时和失败原因
