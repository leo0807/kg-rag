...
function SectionCard({
    ...
    return (
        <section className="rounded-2xl border border-gray-800 bg-gray-950/90 p-5">
            <div className="mb-4">
        ...
        <div className="space-y-6">
            ...
            >
                <div className="space-y-4">
                    ...
                    />
                    <div>
                        <label className="mb-1 block text-xs text-gray-500">提供方</label>
                        <select
                    ...
                    {settings.llm_mode === "local" ? (
                        <FieldGrid
                            settings={settings}
                            onChange={update}
                            fields={[{ key: "llm_model", label: "本地模型名称", placeholder: "qwen2.5:7b" }]}
                        />
                    ) : (
                        <FieldGrid settings={settings} onChange={update} fields={llmApiFields} />
                    )}
                </div>
            </SectionCard>
                ...
                <div className="space-y-4">
                    ...
                    {settings.embedding_mode === "local" ? (
                        <FieldGrid
                            settings={settings}
                            onChange={update}
                            fields={[{ key: "embedding_model", label: "模型路径", placeholder: "models/bge-m3" }]}
                        />
                    ) : (
                        <FieldGrid
                            settings={settings}
                            ...
                            fields={
                                settings.embedding_provider === "qwen"
                                    ? [
                                          { key: "embedding_api_key", label: "API Key", placeholder: "DashScope Key", type: "password" },
                                          { key: "embedding_model", label: "模型名称", placeholder: "text-embedding-v3" },
                                      ]
                                    : [
                                          { key: "embedding_api_url", label: "API 地址", placeholder: "https://api.openai.com/v1" },
                                          { key: "embedding_api_key", label: "API Key", placeholder: "sk-...", type: "password" },
                ...
                <div className="space-y-4">
                    ...
                    {settings.reranker_mode === "local" ? (
                        <FieldGrid
                            settings={settings}
                            onChange={update}
                            fields={[{ key: "reranker_model", label: "模型路径", placeholder: "models/bge-reranker-v2-m3" }]}
                        />
                    ) : (
                        <FieldGrid
                            settings={settings}
                            onChange={update}
                            fields={[
                                { key: "reranker_api_url", label: "API 地址", placeholder: "https://your-service.example.com" },
                                { key: "reranker_api_key", label: "API Key", placeholder: "token / sk-...", type: "password" },
                                { key: "reranker_model", label: "模型名称", placeholder: "bge-reranker-v2-m3" },
                            ]}
                        />
                    )}
                </div>
            ...
            >
                <div className="grid gap-4 lg:grid-cols-2">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="mb-2 block text-xs text-gray-500">视觉服务模式</label>
                            <ModeSwitch
                        ...
                        <div>
                            <label className="mb-2 block text-xs text-gray-500">默认检索策略</label>
                            <div className="flex flex-wrap gap-2">
                                {["parallel", "sequential", "graph_augmented", "multi_hop"].map((strategy) => (
                                    <button
                                        key={strategy}
