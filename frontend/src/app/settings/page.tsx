  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-4 sm:px-6 lg:px-8 lg:py-8">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[240px_1fr] lg:gap-6">
        {/* 左侧导航 */}
        <aside className="bg-gray-950 border border-gray-800 rounded-2xl p-3 h-fit lg:sticky lg:top-4">
          ...
          <div className="space-y-2">
            {tabs.map((t) => (
              <button
                ...
                className={`w-full text-left px-3 py-3 rounded-xl transition-colors min-h-14 ${
                  tab === t.key
        ...
        <section className="space-y-4">
          <div className="bg-gray-950 border border-gray-800 rounded-2xl px-4 py-4 sm:px-5">
          ...
          {msg && (
            <div className="px-3 py-2 bg-green-500/10 border border-green-500/30 rounded-lg text-sm text-green-400">
              {msg}
            </div>
          )}
          {error && (
          ...
          {tab === "profile" && (
            <ProfileTab showMsg={showMsg} showError={showError} />
          )}
          {tab === "password" && (
            <PasswordTab showMsg={showMsg} showError={showError} />
          )}
          {tab === "model" && (
          ...
          {tab === "audit" && <AuditTab />}
          {tab === "search" && (
            <SearchTab showMsg={showMsg} showError={showError} />
          )}
          {tab === "entity_filter" && <EntityFilterTab />}
          {tab === "alert" && (
            <AlertTab showMsg={showMsg} showError={showError} />
          )}
        </section>
      </div>
    </div>
