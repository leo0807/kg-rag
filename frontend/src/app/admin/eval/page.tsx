...
import { Download, FlaskConical, Loader2, Play, RefreshCw, Upload } from "lucide-react";
import { fetchApi } from "@/lib/api";

const API = "http://localhost:8000";

type Strategy = "parallel" | "sequential" | "graph_augmented" | "multi_hop" | "counterfactual";
...
export default function AdminEvalPage() {
    ...
    useEffect(() => () => {
        ...
        if (objectiveTimerRef.current) window.clearInterval(objectiveTimerRef.current);
    }, []);
    ...
    async function handleStart() {
        ...
        try {
            ...
            if (timerRef.current) window.clearInterval(timerRef.current);
            timerRef.current = window.setInterval(() => { loadTask(data.task_id); }, 1500);
        } catch (e) {
    ...
    async function handleStartObjective() {
        ...
        try {
            ...
            if (!res.ok) {
                const body = await res.json().catch(() => ({ detail: "启动客观题测试失败" }));
            ...
            if (objectiveTimerRef.current) window.clearInterval(objectiveTimerRef.current);
            objectiveTimerRef.current = window.setInterval(() => { loadObjectiveTask(data.task_id); }, 1500);
        } catch (e) {
    ...
    const rows = task?.status === "completed" ? task.results : task?.results_preview ?? [];
    const progress = task ? task.completed / Math.max(task.total, 1) : 0;
    const objectiveRows = objectiveTask?.status === "completed" ? objectiveTask.results : objectiveTask?.results_preview ?? [];
    const objectiveProgress = objectiveTask ? objectiveTask.completed / Math.max(objectiveTask.total, 1) : 0;
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        上传 Excel/CSV 测试集，系统将逐条执行智能问答并生成命中率统计与结果明细。
                    </p>
                </div>
                {task && (
                    <button
            ...
            <div className="grid lg:grid-cols-[1.3fr_1fr] gap-6">
                <section className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4">
                    ...
                    <div className="rounded-xl border border-gray-800 bg-gray-950 p-4 text-sm text-gray-400">
                        ...
                        <div className="mt-1">这份你提供的文件表头已匹配，可直接上传运行。</div>
                    </div>
                ...
                <section className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4">
                    ...
                    {!task && <div className="text-sm text-gray-500">尚未启动评测任务。</div>}
                        ...
                        <>
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div className="rounded-xl bg-gray-950 border border-gray-800 p-4">
                                    ...
                                    <div className="mt-1 text-sm text-white truncate">{task.filename}</div>
                                </div>
                                <div className="rounded-xl bg-gray-950 border border-gray-800 p-4">
                                    <div className="text-xs text-gray-500">通过数</div>
                                    <div className="mt-1 text-lg text-emerald-400">{task.passed}</div>
                                </div>
                                <div className="rounded-xl bg-gray-950 border border-gray-800 p-4">
                                    <div className="text-xs text-gray-500">失败数</div>
                                    <div className="mt-1 text-lg text-rose-400">{task.failed}</div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
                                    <div className="h-full bg-indigo-500 transition-all" style={{ width: pct(progress) }} />
                                </div>
                            </div>
                            ...
                            {task.summary && (
                                <div className="rounded-xl bg-emerald-950/20 border border-emerald-800/40 p-4">
                                    ...
                                    <div className="mt-1 text-2xl text-emerald-400 font-semibold">{pct(task.summary.pass_rate)}</div>
                                </div>
                            )}
                ...
                <section className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-gray-800 flex items-center justify-between">
                        ...
                        <div className="text-xs text-gray-500">
                            {task.status === "completed" ? `共 ${task.results.length} 条` : "展示最近执行结果"}
                        </div>
                    </div>
                    ...
                    <div className="overflow-auto">
                        <table className="w-full text-sm min-w-[1100px]">
                            ...
                            <tbody>
                                {rows.map(row => (
                                    <tr key={`${row.row_no}-${row.question}`} className="border-t border-gray-800 align-top">
                                        ...
                                        <td className="px-4 py-3 text-gray-200 whitespace-pre-wrap">{row.question}</td>
                                        <td className="px-4 py-3 text-gray-400 whitespace-pre-wrap">{row.expected_answer}</td>
                                        <td className="px-4 py-3 text-gray-300 whitespace-pre-wrap">{row.actual_answer}</td>
                                ...
                                {rows.length === 0 && (
                                    <tr>
                                        <td colSpan={7} className="px-4 py-10 text-center text-gray-500">暂无结果</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                        ...
                        <div className="text-sm text-gray-500 mt-1">
                            上传 `.doc/.docx/.wps` 客观题文档，系统自动抽题并给出预测答案、依据和来源，不需要标准答案。
                        </div>
                    </div>
                    {objectiveTask && (
                        <button
                ...
                <div className="grid lg:grid-cols-[1.3fr_1fr] gap-6">
                    <section className="rounded-2xl border border-gray-800 bg-gray-950 p-5 space-y-4">
                        ...
                        <div className="rounded-xl border border-gray-800 bg-gray-900 p-4 text-sm text-gray-400 space-y-1">
                            ...
                            <div>输出内容：题号、题干、选项、系统预测答案、依据、引用来源。</div>
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-4 h-10 rounded-lg bg-indigo-600 text-white hover:bg-indigo-500 disabled:opacity-50"
                        >
                            {objectiveStarting ? <Loader2 size={15} className="animate-spin" /> : <Play size={15} />}
                            开始客观题测试
                        </button>
                    </section>

                    <section className="rounded-2xl border border-gray-800 bg-gray-950 p-5 space-y-4">
                        {!objectiveTask && <div className="text-sm text-gray-500">尚未启动客观题任务。</div>}
                        {objectiveTask && (
                            <>
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="rounded-xl bg-gray-900 border border-gray-800 p-4">
                                        <div className="text-xs text-gray-500">任务状态</div>
                                        <div className="mt-1 text-sm text-white">{objectiveTask.status}</div>
                                    </div>
                                    <div className="rounded-xl bg-gray-900 border border-gray-800 p-4">
                                        <div className="text-xs text-gray-500">总题数</div>
                                        <div className="mt-1 text-sm text-white">{objectiveTask.total}</div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    <div className="flex justify-between text-xs text-gray-500 mb-2">
                                        <span>进度</span>
                                        <span>{objectiveTask.completed}/{objectiveTask.total}</span>
                                    </div>
                                    <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
                                        <div className="h-full bg-indigo-500 transition-all" style={{ width: pct(objectiveProgress) }} />
                                    </div>
                                </div>

                                {objectiveTask.current_question && objectiveTask.status === "running" && (
                        ...
                        <table className="w-full text-sm min-w-[1100px]">
                            ...
                            <tbody>
                                {objectiveRows.map(row => (
                                    <tr key={`${row.display_no}-${row.question}`} className="border-t border-gray-800 align-top">
                                        <td className="px-4 py-3 text-gray-500">{row.display_no}</td>
                                        <td className="px-4 py-3 text-gray-400">{row.question_type}</td>
                                        <td className="px-4 py-3 text-gray-200 whitespace-pre-wrap break-words leading-6">{row.question}</td>
                                        <td className="px-4 py-3 text-gray-400 whitespace-pre-wrap break-words leading-6">
                                            {row.options.length > 0 ? row.options.map(opt => `${opt.label}. ${opt.text}`).join("\n") : "—"}
                                        </td>
                                        <td className="px-4 py-3 text-indigo-300 font-medium">{row.predicted_answer || "—"}</td>
                                        <td className="px-4 py-3 text-gray-300 whitespace-pre-wrap break-words leading-6">{row.reason || "—"}</td>
                                        <td className="px-4 py-3 text-gray-500">{row.source_refs.join(", ") || "—"}</td>
                                    </tr>
                                ))}
