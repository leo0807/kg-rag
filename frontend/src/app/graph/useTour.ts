...
import { GraphNode, GraphData, TourStop, NodeFilter } from "./constants";
...
export function useTour(
    ...
    async function startTour() {
        ...
        setTourStops([]); setTourIdx(-1); setTourText(""); setTourTotal(0);
        setTourNodeIds(new Set()); setTourCurrentId(""); setTourData(null);
        tourIdxRef.current = -1; tourTextRef.current = "";
        setTourRunning(true); setTourStreaming(false);
        setNodeFilter("全部");
        ...
        try {
                ...
                for (const line of decoder.decode(value).split("\n")) {
                    ...
                    try {
                        ...
                        if (ev.type === "init") {
                            ...
                            setTourIdx(idx); setTourCurrentId(ev.node_id);
                            setTourText(""); tourTextRef.current = "";
                            setTourStreaming(true);
                            setTourStops(prev => [...prev, { index: idx, node_id: ev.node_id, node: ev.node, explanation: "" }]);
                            ...
                            setTourStreaming(false); setTourRunning(false);
                        }
            ...
            setTourRunning(false); setTourStreaming(false);
        }
    }
    ...
    function stopTour() {
        ...
        setTourRunning(false); setTourStreaming(false); setTourData(null);
        setTourNodeIds(new Set()); setTourCurrentId(""); setTourIdx(-1);
        setTourStops([]); setTourText("");
    }
    ...
    function navigateTour(idx: number) {
        ...
        tourIdxRef.current = idx;
        setTourIdx(idx); setTourCurrentId(stop.node_id);
        setTourText(stop.explanation); tourTextRef.current = stop.explanation;
        zoomToNode(stop.node_id, 200);
    }

    return {
        tourOpen, setTourOpen, tourTopic, setTourTopic,
        tourRunning, tourData, tourStops, tourTotal, tourIdx,
        tourText, tourStreaming, tourNodeIds, tourCurrentId,
        hasPrev: tourIdx > 0,
        hasNext: tourIdx >= 0 && tourIdx < tourStops.length - 1 && !tourStreaming,
        startTour, stopTour, navigateTour,
    };
}
