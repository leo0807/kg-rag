...
function findMatchingNodes(query: string, nodes: GraphNode[]): GraphNode[] {
  const normalizedQuery = normalizeSearchText(query);
...
export default function GraphPage() {
  ...
  function zoomIn() {
    ...
    if (el && zoomRef.current)
      (d3.select(el) as any).transition().call(zoomRef.current.scaleBy, 1.3);
  }
  function zoomOut() {
    const el = activeEl();
    if (el && zoomRef.current)
      (d3.select(el) as any).transition().call(zoomRef.current.scaleBy, 0.7);
  }
  function zoomReset() {
    const el = activeEl();
    if (el && zoomRef.current)
      ...
      window.setTimeout(() => {
        ...
        (d3.select(el) as any)
          ...
          .call(
            ...
            d3.zoomIdentity
              .translate(
                width / 2 - simNode.x * targetScale,
                height / 2 - simNode.y * targetScale,
              )
              .scale(targetScale),
          );
      }, delay);
  ...
  async function expandSection(chunkId: string) {
    ...
    try {
      ...
      setData((prev) => {
        ...
        const newNodes = (children as GraphNode[]).filter(
          (n) => !existingIds.has(n.id),
        );
        const newEdges = children.map((c: GraphNode) => ({
          source: chunkId,
          target: c.id,
          type: "HAS_SUBSECTION",
        }));
  ...
  useEffect(() => {
    fetchApi<{ nodes?: Array<{ chunk_id: string; heat_norm: number }> }>(
      ...
      .then((d: any) => {
        if (d?.nodes?.length)
          setHeatMap(
            new Map(d.nodes.map((n: any) => [n.chunk_id, n.heat_norm])),
          );
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const autoMode: RenderMode =
      nc > 5000 ? "heatmap" : nc > 1000 ? "webgl" : nc > 500 ? "canvas" : "svg";
  ...
  return (
    ...
    >
      <GraphToolbar
        ...
        onToggleTables={() =>
          setLimits((prev) => ({ ...prev, tbl: prev.tbl > 0 ? 0 : 200 }))
        }
        ...
        onToggleEntities={() =>
          setLimits((prev) => ({ ...prev, show_entities: !prev.show_entities }))
        }
          ...
          <div ref={containerRef} className="relative flex-1 overflow-hidden">
            ...
            {renderMode !== "svg" && (
              <div className="absolute top-3 left-3 px-2 py-1 bg-gray-900/80 border border-indigo-700/40 rounded-lg text-xs text-indigo-400 pointer-events-none z-10">
                {renderMode === "webgl" &&
                  `WebGL 模式 · ${filteredNodesRef.current.length} 节点`}
                {renderMode === "canvas" &&
                  `Canvas 模式 · ${filteredNodesRef.current.length} 节点`}
                {renderMode === "heatmap" &&
                  `热力图模式 · ${filteredNodesRef.current.length} 节点（按文档聚类）`}
              </div>
            )}
