...
import { AlertCircle, RefreshCw, Ruler, X } from "lucide-react";
...
export function DrawingViewer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      />
      <div
        className="relative bg-gray-950 border border-gray-800 rounded-2xl w-full max-w-4xl max-h-[90vh]
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-800 shrink-0">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              {image.is_drawing && (
                <span
                  className="px-1.5 py-0.5 bg-indigo-900/50 text-indigo-300 border border-indigo-700/40
            ...
            <div className="text-xs text-gray-500 mt-0.5 truncate">
              {image.section_number ? `§${image.section_number}` : "未归属章节"}
              {image.section_title ? ` ${image.section_title}` : ""}
            </div>
          </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-auto bg-gray-900 flex items-center justify-center p-4">
            ...
            <span hidden={!!imgSrc} className="text-sm text-gray-500">
              图片暂不可用
            </span>
          </div>
            ...
            {image.part_numbers.length > 0 && (
              <div className="px-4 py-3 border-b border-gray-800">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {image.part_numbers.map((pn) => (
                    <span
                      key={pn}
                      className="px-2 py-0.5 bg-gray-800 border border-gray-700
              ...
              <div className="px-4 py-3 border-b border-gray-800">
                ...
                <div className="space-y-2">
                  {image.annotations.map((ann) => (
                    <div
                      key={`${ann.type}-${ann.raw}-${ann.parameter}-${ann.value}-${ann.value_max}-${ann.value_min}`}
                      className={`px-3 py-2 rounded-lg border text-xs ${TYPE_COLOR[ann.type] ?? TYPE_COLOR.other}`}
                    >
                      <div className="flex items-center justify-between gap-2 mb-0.5">
                        <span className="font-medium truncate">
                          {ann.parameter || ann.raw}
                        </span>
                      ...
                      <div className="font-mono text-sm">
                        {ann.raw}
                        {ann.unit && (
                          <span className="ml-1 opacity-60">{ann.unit}</span>
                        )}
                      </div>
            ...
            {image.assembly_relations.length > 0 && (
              <div className="px-4 py-3">
                ...
                <ul className="space-y-1.5">
                  {image.assembly_relations.map((rel) => (
                    <li
                      key={rel}
