...
export function KnowledgeCapturePanel({ answerText }: Props) {
    ...
    async function capture() {
        ...
        try {
            await fetchApi("/api/knowledge/capture", {
                method: "POST", body: JSON.stringify({ triples: chosen }),
            });
            setDone(true);
    ...
    return (
        <div className="mt-2 border border-gray-700/60 rounded-lg overflow-hidden">
            <button onClick={() => { setOpen(v => !v); if (!open && triples.length === 0) extract(); }}
            ...
            {open && (
                <div className="bg-gray-900/80 px-3 py-2">
                    ...
                    {!loading && !done && triples.length === 0 && (
                        <button onClick={extract} className="text-xs text-indigo-400 hover:text-indigo-300 py-1">重新分析</button>
                    )}
                    {!loading && !done && triples.length > 0 && (
                        <>
                            <div className="flex items-center justify-between mb-2">
                                <span className="text-[11px] text-gray-500">发现 {triples.length} 个三元组</span>
                                <button onClick={toggleAll} className="text-[11px] text-indigo-400 hover:text-indigo-300">
                            ...
                            <button onClick={capture} disabled={saving || selected.size === 0}
