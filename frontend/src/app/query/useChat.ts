...
export function useChat() {
  ...
  const [filtersByConv, setFiltersByConv] = useState<Record<string, SourcePanelFilters>>(() => {
    if (typeof window === "undefined") return {};
    try { const raw = sessionStorage.getItem(SOURCE_FILTERS_KEY); return raw ? JSON.parse(raw) : {}; }
    catch { return {}; }
  });
  ...
  const stream = useStreamQuery({
    ...
    conversations: conversations.conversations, createConversation, updateConversation, showNetToast,
  });
  ...
  useEffect(() => { deleteConvRef.current = conversations.deleteConversation; });
  ...
  const activeSourceFilters = activeId ? (filtersByConv[activeId] ?? defaultFilters()) : defaultFilters();
  ...
  function toggleCompareMode() {
    setCompareMode((v) => { const next = !v; localStorage.setItem("query:compare_mode", next ? "1" : "0"); return next; });
  }

  async function handleSubmit() {
    if ((!input.trim() && pendingImages.length === 0) || stream.loading || stream.streaming) return;
  ...
  return {
    ...
    quoteSource, setQuoteSource,
    netToast, setNetToast,
