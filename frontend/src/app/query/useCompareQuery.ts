...
export function useCompareQuery() {
    const [state, setState] = useState<CompareState>({
        loading: false, retryingStrategy: null, question: "", results: [],
    });
    ...
    async function compare(question: string) {
        ...
        try {
            ...
            const res = await fetch("http://localhost:8000/api/query/compare", {
                method:  "POST",
            ...
            setState({ loading: false, retryingStrategy: null, question, results: data.results ?? [] });
        } catch (e) {
        ...
        try {
            ...
            if (newResult) {
                setState(prev => ({
                    ...
                    results: prev.results.map(r => r.strategy === strategy ? newResult : r),
                }));
            } else {
