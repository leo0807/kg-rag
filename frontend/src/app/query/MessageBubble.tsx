  ...
  if (role === "user") {
    return (
      <div className="flex justify-end mb-6">
        <div className="max-w-[75%] flex flex-col items-end gap-2 relative group">
          {onEditQuestion && (
            <button
              type="button"
              onClick={onEditQuestion}
              className="absolute top-2 right-2 rounded-full border border-gray-800 bg-gray-950 p-1.5 text-gray-500 opacity-0 shadow-lg transition-all hover:border-indigo-500 hover:text-indigo-300 group-hover:opacity-100"
              title="编辑重答"
              aria-label="编辑重答"
            >
              <PencilLine size={12} />
          ...
          {content && (
            <div className="px-4 py-3 pr-10 bg-[#1B6BB5] rounded-2xl rounded-tr-sm">
    ...
    <div className="flex justify-start mb-6">
      <div className="max-w-[85%] w-full">
        <div className="flex items-center gap-2 mb-2">
          <div className="w-6 h-6 rounded-full bg-[#1B6BB5]/20 border border-[#1B6BB5]/30 flex items-center justify-center text-xs text-[#1B6BB5] dark:text-[#5BA3E0] font-bold">
            AI
          </div>
        ...
        {!errorInfo && (
          <div className="px-4 py-3 bg-gray-900 border border-gray-800 rounded-2xl rounded-tl-sm">
            ...
            {!streaming && content && (
              <KnowledgeCapturePanel answerText={content} />
            )}
            {!streaming && question && (
