...
export default function QueryPage() {
  ...
  const [compareMode, setCompareMode] = useState(() => {
    if (typeof window !== "undefined")
      return localStorage.getItem("query:compare_mode") === "1";
    return false;
  });
  const [
    sourcePanelFiltersByConversation,
    setSourcePanelFiltersByConversation,
  ] = useState<Record<string, SourcePanelFilters>>(() => {
  ...
  const favoritedChunkIds = new Set(
    favorites
      .filter((f) => f.type === "section" && f.section_id)
      .map((f) => f.section_id!),
  );
  const bottomRef = useRef<HTMLDivElement>(null);
  const toastTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  function showNetToast(
    type: NetToastType,
    label: string,
  ...
  useEffect(() => {
    deleteConvRef.current = deleteConversation;
  });
  ...
  const activeSourcePanelFilters = activeId
    ? (sourcePanelFiltersByConversation[activeId] ??
  ...
  async function handleSubmit() {
    if ((!input.trim() && pendingImages.length === 0) || loading || streaming)
      return;
                ...
                {activeConv.messages.map((msg, i) => (
                  <div key={msg.id}>
                    ...
                    <MessageBubble
                      ...
                      errorInfo={
                        msg.role === "assistant" ? msg.errorInfo : undefined
                      }
                      isAdmin={isAdmin}
                      ...
                      sourcePanelFilters={
                        msg.role === "assistant"
                          ? activeSourcePanelFilters
                          : undefined
                      }
                      onSourcePanelFiltersChange={
                        msg.role === "assistant"
                          ? handleSourcePanelFiltersChange
                          : undefined
                      }
