...
describe("QuotaPanel", () => {
  ...
  it("renders quota rows after successful fetch", async () => {
    ...
    render(<QuotaPanel />);
    await waitFor(() =>
      expect(screen.queryByText(/加载配额数据/)).not.toBeInTheDocument(),
    );
  ...
  it("shows 403 error message for permission denied", async () => {
    ...
    render(<QuotaPanel />);
    await waitFor(() =>
      expect(screen.getByText("无权限查看配额")).toBeInTheDocument(),
    );
  });
  ...
  it("shows generic error for non-403 failures", async () => {
    ...
    render(<QuotaPanel />);
    await waitFor(() =>
      expect(screen.getByText("配额数据加载失败")).toBeInTheDocument(),
    );
  });
