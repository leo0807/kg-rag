...
describe("HelpDrawer", () => {
    ...
    it("renders trigger button", () => {
        render(<HelpDrawer />);
        expect(screen.getByRole("button", { name: "打开页面帮助" })).toBeInTheDocument();
    });

    it("trigger button has aria-expanded false when drawer is closed", () => {
        render(<HelpDrawer />);
        expect(screen.getByRole("button", { name: "打开页面帮助" })).toHaveAttribute("aria-expanded", "false");
    });
    ...
    it("opens drawer on trigger button click (aria-expanded becomes true)", async () => {
        ...
        expect(screen.getByRole("button", { name: "打开页面帮助" })).toHaveAttribute("aria-expanded", "true");
    });
    ...
    it("does not render trigger when no guide found", () => {
        ...
        render(<HelpDrawer />);
        expect(screen.queryByRole("button", { name: "打开页面帮助" })).not.toBeInTheDocument();
    });
