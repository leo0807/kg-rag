...
describe("BillingPage", () => {
  ...
  it("renders bills table after successful fetch", async () => {
    mockFetchApi.mockResolvedValueOnce(BILLS).mockResolvedValueOnce(PLANS);
    render(<BillingPage />);
    await waitFor(() =>
  ...
  it("shows 403 permission error", async () => {
    type ApiErrorCtor = new (msg: string, status: number) => Error;
    mockFetchApi.mockRejectedValue(
      new (ApiError as unknown as ApiErrorCtor)("Forbidden", 403),
    );
    render(<BillingPage />);
    await waitFor(() =>
      expect(screen.getByText("无权限访问账单功能")).toBeInTheDocument(),
    );
  });
  ...
  it("pending bill shows 标记已付 button", async () => {
    ...
    render(<BillingPage />);
    await waitFor(() =>
      expect(screen.getByText("标记已付")).toBeInTheDocument(),
    );
  });

  it("paid bill does not show 标记已付 button", async () => {
    const paidOnly = [BILLS[0]]; // status: paid
    mockFetchApi.mockResolvedValueOnce(paidOnly).mockResolvedValueOnce(PLANS);
    render(<BillingPage />);
    await waitFor(() =>
      expect(screen.queryByText(/加载中/)).not.toBeInTheDocument(),
    );
  ...
  it("shows empty state when no bills", async () => {
    mockFetchApi.mockResolvedValueOnce([]).mockResolvedValueOnce([]);
    render(<BillingPage />);
    await waitFor(() =>
