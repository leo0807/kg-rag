...
- [x] **密钥与凭证管理**：当前 JWT_SECRET、数据库密码等均为 `.env` 明文，集成 HashiCorp Vault 或云端 Secrets Manager（AWS Secrets Manager / GCP Secret Manager），并在启动时强制校验非默认值
- [x] **传输层加密**：docker-compose 中各服务间通信未启用 TLS；生产部署需为 Neo4j、PostgreSQL、Redis、Elasticsearch 配置 TLS 证书
- [x] **Redis 认证**：当前 Redis 无密码，需启用 `requirepass` 并在连接串中配置
- [x] **Elasticsearch 安全模式**：当前 `xpack.security.enabled=false`，生产需启用 xpack 鉴权与 TLS
- [x] **文件上传防护**：`/api/ingest` 无文件大小上限与类型校验，需加 `max_size`（如 200MB）及 MIME 类型白名单（仅 PDF）
- [x] **请求体大小限制**：FastAPI 全局配置 `max_request_body_size`，防止超大 JSON 攻击
- [x] **依赖漏洞扫描**：集成 `pip-audit`（Python）+ `npm audit`（前端）定期扫描已知 CVE

---

### CI/CD 流水线

- [x] **后端测试流水线**：GitHub Actions `test.yml`，在每个 PR 上自动运行 `pytest tests/ -v --cov`，覆盖率低于阈值时阻断合并
- [x] **前端测试流水线**：GitHub Actions 运行 `vitest run` + `biome check`（lint/format），失败时阻断合并
- [x] **Docker 镜像自动构建**：合并至 main 分支时自动构建并推送镜像至 Docker Registry（GHCR 或私有仓库）
- [x] **安全扫描**：`security.yml` 中集成 Trivy（镜像漏洞扫描）+ `git-secrets`（防止密钥入库）
- [x] **语义化版本与 Changelog**：集成 `semantic-release`，根据 commit message 自动生成版本号和 CHANGELOG.md
...
- [x] **OpenTelemetry 分布式追踪**：集成 `opentelemetry-sdk`，自动 instrument FastAPI / SQLAlchemy / httpx，导出至 Jaeger 或 Grafana Tempo
- [x] **Prometheus 指标暴露**：集成 `starlette-prometheus`，暴露 `/metrics` 端点，包含 QPS、延迟分位数、缓存命中率、LLM token 消耗等指标
- [x] **Grafana 仪表盘**：基于 Prometheus 指标搭建运营大盘（查询成功率、检索延迟 P50/P99、向量库 QPS、LLM 费用趋势）
- [x] **告警规则**：配置 Alertmanager 规则，在服务宕机、错误率 > 5%、P99 延迟 > 5s 时触发告警（钉钉 / 企业微信 webhook）
...
- [x] **LLM API 重试与熔断**：使用 `tenacity` 对 LLM API 调用实现指数退避重试（最多 3 次），并用 `pybreaker` 实现熔断，防止级联失败
- [x] **向量库 / 图数据库连接池健康检查**：启动时及运行时定期 ping，连接失败时降级（仅全文检索）而非直接 500
- [x] **异步任务队列**：将 PDF 入库（耗时 > 30s）迁移至 Celery + Redis 队列，支持任务重试、失败重新入队，前端通过 WebSocket 订阅进度（当前同步阻塞）
...
- [x] **文档权限模型**：为 Document 表添加 `owner_id` + `visibility`（private / department / public），用户只能检索有权限的文档
- [x] **对话隔离**：Conversation 查询时强制过滤 `user_id = current_user.id`，防止越权读取他人历史
- [x] **部门级知识库隔离**：支持按部门（department）划分文档访问范围，管理员可配置跨部门共享
- [x] **资源配额**：每用户每天查询次数上限（当前仅全局限流），存储配额（上传文档大小/数量），超限返回 429 并提示

---

### API 工程化

- [x] **API 版本管理**：所有路由迁移至 `/api/v1/` 前缀，旧路径保留 6 个月并返回 `Deprecation` 响应头，为未来破坏性变更预留空间
- [x] **分页一致性**：当前 `/api/documents` 用 `page/per_page`，其余接口用不同参数名；统一为 `cursor` 游标分页，支持大数据集无损翻页
- [x] **幂等性保障**：`POST /api/ingest` 等写操作支持 `Idempotency-Key` 请求头，避免网络超时后客户端重试造成重复入库
- [x] **OpenAPI 客户端生成**：发布 `openapi.json`，并在 CI 中自动生成 Python / TypeScript SDK 供内部系统集成
...
- [x] **Kubernetes 部署清单**：将 docker-compose.yml 转换为 Helm Chart（`charts/kg-rag/`），包含 Deployment / Service / PVC / ConfigMap / Secret 模板
- [x] **多环境配置分离**：`docker-compose.dev.yml` / `docker-compose.staging.yml` / `docker-compose.prod.yml`，各环境独立的资源限制、副本数、镜像标签
- [x] **自动扩缩容（HPA）**：Kubernetes HPA 基于 CPU / 自定义 QPS 指标自动扩展 FastAPI 副本（1-10 个）
- [x] **备份与恢复**：PostgreSQL 每日自动备份至对象存储（`pg_dump` + S3/MinIO），Neo4j 增量备份脚本，定期演练恢复流程
- [x] **灾难恢复演练文档**：记录 RTO（恢复时间目标）、RPO（恢复点目标），以及各服务故障时的降级策略

---

### 性能优化

- [x] **Embedding 批处理**：当前每条 section 逐一调用 `encode()`，迁移至 `encode(batch)` 并发处理，入库速度预计提升 5-10×
- [x] **向量索引离线构建**：大规模入库时暂停在线索引更新（Milvus `disable_index`），批量写入后重建，避免实时索引影响写入吞吐
- [x] **Neo4j 查询缓存**：对高频只读 Cypher 查询（图谱统计、实体列表）添加应用层 Redis 缓存（TTL 5 分钟）
- [x] **前端资源优化**：D3.js 图谱渲染节点超 500 时启用 Canvas 模式替代 SVG，避免 DOM 膨胀导致浏览器卡顿
- [x] **流式响应背压控制**：当前 SSE 无限速，LLM 生成过快时前端可能积压；添加服务端速率控制（字符/秒上限可配置）
...
- [x] **混沌工程**：模拟 Neo4j / Milvus 宕机时系统的降级行为（预期：返回全文检索结果而非 500）
- [x] **前端 E2E 测试**：Playwright 覆盖登录 → 上传文档 → 提问 → 查看图谱的完整用户旅程
- [x] **安全渗透测试**：OWASP ZAP 自动扫描 + 手工测试 SQL 注入、XSS、IDOR（越权读取他人对话）
- [x] **LLM 评估基准**：构建 50 条 "问题-标准答案" 对，在每次模型/策略变更后自动计算 BLEU / ROUGE / 人工评分，防止效果回归
...
- [x] **工艺完整性校验**：检测工序图中的孤立节点（有 Section 但无 Tool / Material / Process 关联），生成"实体提取不完整"报告，辅助数据质量改进
- [x] **悬空引用检测**：扫描所有 `REFERENCES` 边，若目标文档不在图谱中则标记为悬空引用，提示管理员补充入库
- [x] **循环依赖检测**：检测工序先后关系（`PRECEDES`）中是否存在环路（A→B→C→A），防止工艺流程逻辑错误
- [x] **版本一致性检查**：检测同一设备型号下，不同版本文档之间 Constraint 值的漂移趋势，自动生成版本对比报告
...
- [x] 新增 `mcp_server/` 目录，使用 `mcp` Python SDK（`pip install mcp`）实现 `StdioServer`
- [x] 每个 Tool 对应一个 `@server.tool()` 装饰函数，内部调用现有 FastAPI 服务
- [x] 发布 `claude_desktop_config.json` 示例，用户一键配置即可在 Claude Desktop 中使用
- [x] 支持 SSE 传输模式（`mcp[sse]`），允许 Web 端 MCP 客户端流式接收问答结果
- [x] MCP Resources 扩展：将文档列表暴露为 `resource://documents/{doc_id}`，AI 可直接"阅读"原始规范文本
...
- [x] 在 `backend/src/skills/` 下实现各 Skill 的业务逻辑，独立于 RAG 检索管线
- [x] 在 `multi_hop.py` 中将 LangGraph Agent 的工具列表升级为 Skills 集，替代硬编码子问题拆解
- [x] 前端"策略"下拉新增 `agent` 选项，调用 `POST /api/query/agent`，后端以 ReAct 循环执行 Tool 调用直到得出最终答案
- [x] 工具调用日志写入 Langfuse span，可视化每次推理的 Tool 调用链路和中间结果
...
- [x] **对外暴露 Agent Card**：在 `/.well-known/agent.json` 发布标准 Agent Card，描述本系统的能力边界（支持的任务类型、输入输出格式、认证方式）
- [x] **接收任务委托**：实现 `POST /api/a2a/tasks/send` 端点，接收其他 Agent 以 A2A 格式委托的问答或图谱查询任务，返回结构化结果
- [x] **主动委托子任务**：当本系统判断问题超出工艺规范范围（如涉及 ERP 库存数据），通过 A2A 协议将子任务委托给企业内网的 ERP Query Agent
- [x] **流式任务状态**：支持 A2A 的 `tasks/sendSubscribe` 流式端点，让调用方实时获取推理进度（类似当前的 SSE）
...
- [x] **实体提取优化**：当前 `entity_extractor.py` 依赖手写 Prompt；用 DSPy `ChainOfThought` 模块 + 50 条标注样本自动优化提取准确率
- [x] **Reranker 分数校准**：以 `(query, chunk, relevance_label)` 三元组训练 DSPy 评分模块，替代固定阈值的硬截断逻辑
- [x] **Text2Cypher 生成**：训练 DSPy 模块将自然语言问题翻译为 Cypher 查询，自动优化生成准确率（基于图谱执行结果的自动反馈）
- [x] **答案质量评估**：DSPy `Assess` 模块自动评估 LLM 答案的忠实度（Faithfulness）和相关性（Relevance），替代人工抽检
...
pip install dspy-ai
```
- [x] `scripts/dspy_optimize_entity.py` — 实体提取 Prompt 优化脚本，输出最优 Prompt 写入 `config/prompts/entity.json`
- [x] `scripts/dspy_optimize_cypher.py` — Text2Cypher 优化，基于图谱执行成功率自动反馈
...
- [x] **系统提示缓存**：当前每次查询都重复发送约 800 token 的系统提示（角色定义 + 输出格式要求），启用 `cache_control: {"type": "ephemeral"}` 后缓存命中时费用接近零
- [x] **长文档上下文缓存**：当同一章节被多次不同问题引用时，缓存该章节的 token 表示，避免重复编码（适用于热点章节）
- [x] **Few-shot 示例缓存**：将固定的 few-shot 示例（实体提取、答案格式）写入缓存前缀，所有请求共享缓存
- [x] **费用追踪区分**：在 `llm_usage` 表新增 `cache_read_tokens` / `cache_write_tokens` 字段，在成本报表中单独展示缓存节省金额
...
- [x] `scripts/ragas_eval.py` — 加载 50 条人工标注 QA 对，调用各检索策略，计算 RAGAS 四项指标并写入 `eval_results.json`
- [x] GitHub Actions `eval.yml` — 每次合并至 main 时自动运行评估，若任一指标低于基线则在 PR 评论中告警
- [x] 管理后台新增"评估报告"页，展示各策略（parallel / gnn / multi_hop）的 RAGAS 指标历史趋势折线图
- [x] TruLens 集成：对每次生产查询进行在线评分（Groundedness + Answer Relevance），写入 PostgreSQL，异常低分查询自动加入人工复核队列
...
- [x] `scripts/export_finetune_data.py` — 从 `query_feedback`（rating=1）导出 SFT 格式数据集（Alpaca / ShareGPT 格式）
- [x] `scripts/finetune_qwen.py` — 使用 LLaMA-Factory 或 Axolotl 对 Qwen2.5-7B 进行 LoRA 微调，训练数据为本系统积累的航空工艺问答对
- [x] 微调后模型通过 Ollama 本地部署，在 `.env` 中切换 `LLM_MODEL` 即可无缝对接（系统已支持 OpenAI 兼容 API）
- [x] A/B 测试框架：50% 流量走微调本地模型，50% 走原始 API，对比 RAGAS 指标和用户评分，验证蒸馏效果
- [x] Reranker 微调：以 `(query, positive_chunk, negative_chunk)` 三元组微调 `bge-reranker-v2-m3`，提升航空术语的精排准确率
...
- [x] 构造 RAFT 数据集：每条样本包含 1 个相关章节 + 3 个干扰章节 + 标准答案（含 `<citation>` 标注）
- [x] 与标准 SFT 数据集分批训练，对比 Faithfulness 指标，验证抗干扰能力提升效果

**ColBERT / ColPali 晚交互检索**

- [x] 集成 ColBERT v2：不对 query 和 document 编码为单一向量，而是逐 token 交互后取最大相似度，提升长文本精确匹配
- [x] ColPali：直接对 PDF 页面图像编码（Vision-Language Model），无需文字提取即可检索图文混排技术文档
...
- [x] 部署 Debezium PostgreSQL Connector，捕获 `conversations` / `query_feedback` / `llm_usage` 表的变更事件
- [x] 变更事件流入 Kafka，下游数据仓库（ClickHouse / Apache Doris）实时消费，支持亚秒级报表刷新

**Apache Flink — 实时图谱分析**

- [x] 实时计算热点节点（滑动窗口 1 小时内被引用最多的 Section），动态更新 `Section.heat_score` 属性
- [x] 实时检测约束违规流：OPC-UA 数据流 → Flink CEP（复杂事件处理）→ 检测"连续 3 次超限"模式 → 触发告警
...
基于对项目现状的了解，按价值维度分类：                                                                                                
  ...
  评估体系                                                                                                                              
  - 构建标注问答集，自动计算 Recall@K / MRR / NDCG，量化检索效果                                                                        
  - 答案忠实度检测（Faithfulness）：判断回答是否真实来自召回段落，防止幻觉                                                              
  - A/B 测试框架：对比不同策略/模型在真实问题上的表现                                                                                   
                                                                                                                                        
  更好的分块策略                                                                                                                        
  - 当前按章节固定分块，可改为语义分块（按段落语义完整性切割）                                                                          
  - 表格和图片单独建索引（已有提取，但向量化不完整）                                                                                    
                                                                                                                                        
  领域微调                                                                                                                              
  - 用航空工艺规范数据微调 Embedding 模型，提升专业术语召回率                                                                           
  - LoRA 微调 LLM，使回答风格更贴近工艺规范语言                                                                                         
                                                                                                                                        
  ---                                                                                                                                   
  二、知识图谱增强                                          
                                                                                                                                        
  - 关系补全：基于现有图谱训练链接预测模型，推断隐含关系    
  - 图谱版本管理：文档更新时追踪实体/关系变化，支持历史版本对比                                                                         
  - 规范冲突检测：自动发现同一约束在不同文档中的矛盾定义                                                                                
  - 知识地图：以文档/章节为节点，按引用关系生成可视化依赖图                                                                             
                                                                                                                                        
  ---                                                                                                                                   
  三、文档处理能力                                                                                                                      
                                                            
  - 公式识别：formula_service.py 已存在但未完整集成，LaTeX 渲染和向量化
  - CAD 图纸语义理解：当前只做 OCR，可结合 VLM 提取尺寸标注含义                                                                         
  - 增量更新：文档修订时只重处理变更章节，而非全量重入库                                                                                
  - 多语言支持：部分规范可能有英文版本，跨语言检索                                                                                      
                                                                                                                                        
  ---                                                                                                                                   
  四、用户功能                                                                                                                          
                                                            
  - 报告生成：选定若干章节 + 问题，一键生成 Word/PDF 工艺摘要报告
  - 差异对比：同一规范不同版本的章节差异高亮（/compare 已有雏形）                                                                       
  - 协作标注：多人对同一段落的批注、争议、确认状态（annotations 已有基础）                                                              
  - 知识订阅：特定文档/实体更新时推送通知                                                                                               
  - 离线模式：PWA 支持，缓存常用规范供无网络环境使用                                                                                    
                                                                                                                                        
  ---                                                                                                                                   
  五、工程基础设施                                                                                                                      
                                                            
  - 完整 CI/CD：GitHub Actions 自动测试 + Docker 镜像构建 + 部署
  - 全链路监控：已接入 Langfuse 追踪 LLM 调用，可扩展到检索耗时、缓存命中率的 Grafana 大盘                                              
  - backend 纳入 Docker Compose：当前 compose 只有基础设施，应用层靠手动启动                                                            
  - 数据库迁移工具：引入 Alembic 管理 PostgreSQL schema 变更，当前靠 init_tables 暴力建表                                               
                                                                                                                                        
  ---                                                                                                                                   
  六、安全与合规                                                                                                                        
                                                            
  - 字段级权限：不同部门只能看各自文档，而非全库（当前 department 字段已采集但未用于过滤）
  - 操作审计日志完善：已有 audit_cleanup 任务，但写入点不完整                                                                           
  - 敏感内容脱敏：导出/分享时自动屏蔽涉密字段                                                                                           
  ...
  │ 优先级 │     方向     │              理由              │                                                                            
  ├────────┼──────────────┼────────────────────────────────┤                                                                            
  │ 1      │ 评估体系     │ 没有量化就无法判断优化是否有效 │
  ├────────┼──────────────┼────────────────────────────────┤                                                                            
  │ 2      │ 规范冲突检测 │ 航空场景独特价值，竞品没有     │                                                                            
  ├────────┼──────────────┼────────────────────────────────┤                                                                            
  │ 3      │ 报告生成     │ 直接产出物，用户感知最强       │                                                                            
  ├────────┼──────────────┼────────────────────────────────┤                                                                            
  │ 4      │ CI/CD + 监控 │ 支撑后续迭代稳定性             │
  ├────────┼──────────────┼────────────────────────────────┤                                                                            
  │ 5      │ 字段级权限   │ 企业落地必要条件               │
  └────────┴──────────────┴────────────────────────────────┘      